  ...
  try {
    ...
    switch (route) {
      ...
      case "user":
        ...
        break;
        case "allUsers":
          data = await getAllUsers(accessToken); // get signed in user's Microsoft 365 data
          break;
          case "learningdata":
            data = await  getLearningData(accessToken); // get signed in user's Microsoft 365 data
            break;
...
async function getAccessToken(ssoToken) {
  try {
    ...
    urlencoded.append('scope', "User.Read User.ReadBasic.All User.Read.All LearningProvider.Read ");
...
async function getAllUsers(accessToken) {
  ...
  let allUsersEndpoint = "https://graph.microsoft.com/v1.0/users"
  let allUsersOptions = {
      method: 'GET',
      headers: {
          'Authorization': `Bearer ${accessToken}`,
...
async function getLearningData(accessToken) {
  let userEndpoint = "https://graph.microsoft.com/v1.0/employeeExperience/learningProviders"
  let userOptions = {
      method: 'GET',
      headers: {
          'Authorization': `Bearer ${accessToken}`,
  ...
  try {
      ...
      if (response.ok) {
          ...
          console.log('learning data error')
          throw { status: 500, message: "Unable to get user's Microsoft 365 data." };
      }
  } catch (err) {
